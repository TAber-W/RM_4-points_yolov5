...
from models.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, C3, ShuffleV2Block, Concat, NMS, autoShape, StemBlock, BlazeBlock, DoubleBlazeBlock,h_sigmoid, h_swish, SELayer, conv_bn_hswish, MobileNet_Block
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.no = nc + 5 + 8  # number of outputs per anchor #change_1
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = torch.full_like(x[i], 0)
                class_range = list(range(5)) + list(range(13,13+self.nc)) #change_2
                y[..., class_range] = x[i][..., class_range].sigmoid()
                y[..., 5:13] = x[i][..., 5:13] #change_3
                #y = x[i].sigmoid()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3, ShuffleV2Block, StemBlock, BlazeBlock, DoubleBlazeBlock,h_sigmoid, h_swish, SELayer, conv_bn_hswish, MobileNet_Block]:
            c1, c2 = ch[f], args[0]
